...
| **LinkedIn (browser)** | *Experimental* – Playwright can still be flagged as a bot (headless mode is disabled so you see what’s happening) | ***Typical failure modes***<br><br>**1) Start-up login loop** – LinkedIn stalls on “Logging in … [Your Name]”.<br> • Playwright pops up its debug window and pauses.<br> • Click the normal login button or re-enter your password, then hit **Resume** in the Playwright toolbar.<br><br>**2) Mid-scrape bot detection** – selectors stop matching and the crawl freezes.<br> • Close the Streamlit app, relaunch, and repeat steps from *Start-up login loop* when prompted.<br> • Happens mainly after many consecutive searches or from less-trusted IPs.<br><br>**Better solution:** a rotating residential-proxy layer could cut down on Linkedin rate-limits and make full restarts unnecessary |
